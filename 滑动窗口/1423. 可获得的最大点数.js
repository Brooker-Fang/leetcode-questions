 ...
 * @return {number}
 * 思路：
 *  每一步拿牌都是从头或者尾拿, 所以最终剩余的牌一定是连续的, 所以即求连续的最小值，即窗口大小为 cardPoints.length - k 的最小值
 */
var maxScore = function (cardPoints, k) {
  const totalSum = cardPoints.reduce((val, cur) => val + cur, 0);
  let right = 0,
    left = 0,
    sum = 0,
    min = Number.MAX_VALUE,
    len = cardPoints.length - k;
  ...
  console.info(totalSum - min);
  return totalSum - min;
};
maxScore([1, 2, 3, 4, 5, 6, 1], 3); // 12
maxScore([2, 2, 2], 2); // 4
maxScore([9, 7, 7, 9, 7, 7, 9], 7); // 55
maxScore([1, 1000, 1], 1); // 1
maxScore([1, 79, 80, 1, 1, 1, 200, 1], 3); // 202
